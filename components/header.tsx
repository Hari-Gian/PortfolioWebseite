...
const Navbar = () => {
    ...
    return (
        <>
            <nav className="fixed top-0 left-0 w-full z-50 bg-black/25 backdrop-blur-sm border-b border-white/50">
                <div className="max-w-6xl mx-auto flex items-center justify-between p-4">
                    ...
                    >
                        <div className="relative w-6 h-6">
                            <div
                                className={`absolute w-6 h-0.5 bg-current transition-all duration-300 ${nav ? "rotate-45 top-3" : "top-1"}`}/>
                            <div
                                className={`absolute w-6 h-0.5 bg-current transition-all duration-300 ${nav ? "opacity-0" : "top-3"}`}/>
                            <div
                                className={`absolute w-6 h-0.5 bg-current transition-all duration-300 ${nav ? "-rotate-45 top-3" : "top-5"}`}/>
                        </div>
                    </button>
                    ...
                    <ul className="hidden sm:flex space-x-8">
                        {navItems.map(({label, href}) => (
                            <li key={label}>
                                <a
                                    href={href}
                                    className="text-gray-300 hover:text-white transition-colors px-6 py-4"
                                >
                                    {label}
                                </a>
                            </li>
